    ...
    case "UserPromptSubmit": {
      ...
      if (promptText && typeof promptText === "string") {
        ...
        if (trimmed) {
          ...
          // "{cwd_basename}" or "Session {id8}" label with the readable prompt,
          // but only while the name still looks auto-generated (never seeded from
          // a prompt before). We detect that via a NULL/blank name, the
          // "Session {id8}" default written by ensureSession, or the cwd-basename
          // written at session creation time.
          const nameLabel = trimmed.slice(0, 60);
          const curSession = stmts.getSession.get(sessionId);
          const cwdBasename = curSession?.cwd ? require("path").basename(curSession.cwd) : null;
          const looksAutoGenerated =
            !curSession?.name ||
            curSession.name === `Session ${sessionId.slice(0, 8)}` ||
            curSession.name === `Session ${sessionId.slice(0, 8)}`.trim() ||
            (cwdBasename !== null && curSession.name === cwdBasename);
